...
The Spring MVC TestMain framework builds on the mock Servlet API objects to provide an
...
<<spring-mvc-test-framework, Spring MVC TestMain Framework>> instead.
...
TestMain classes typically declare either an array of resource locations for XML or Groovy
...
[[testing-fixture-di]]
==== Dependency Injection of TestMain Fixtures
...
TestMain property sources have higher precedence than those loaded from the operating
...
Spring's `@Timed` annotation has different semantics than JUnit 4's `@TestMain(timeout=...)`
support. Specifically, due to the manner in which JUnit 4 handles test execution timeouts
(that is, by executing the test method in a separate `Thread`), `@TestMain(timeout=...)`
...
Since JUnit Jupiter supports the use of `@TestMain`, `@RepeatedTest`, `ParameterizedTest`,
...
the `@TestMain` and `@Tag` annotations from JUnit Jupiter with the `@Transactional`
...
----
	...
	@TestMain // org.junit.jupiter.api.TestMain
...
* TestMain instance post-processing.
...
for which it is responsible. TestMain instances do not automatically receive access to the
...
TestMain classes that use the TestContext framework do not need to extend any particular
...
[[testcontext-ctx-management-property-sources]]
===== Context Configuration with TestMain Property Sources
...
====== Declaring TestMain Property Sources
...
====== Inheriting and Overriding TestMain Property Sources
...
.TestMain suites and forked processes
[NOTE]
====
...
===== TestMain-managed Transactions

TestMain-managed transactions are transactions that are managed declaratively by using the
...
hierarchy runs within a transaction. TestMain methods that are not annotated with
...
----
	...
	@Transactional
	@TestMain // no expected exception!
...
`@BeforeAll`, `@AfterAll`, `@BeforeEach`, `@AfterEach`, `@TestMain`, `@RepeatedTest`,
...
[[spring-mvc-test-framework]]
=== Spring MVC TestMain Framework

The Spring MVC TestMain framework provides first class support for testing Spring MVC code
...
Spring MVC TestMain also provides client-side support for testing code that uses the
...
The goal of Spring MVC TestMain is to provide an effective way to test controllers by
performing requests and generating responses through the actual `DispatcherServlet`.

Spring MVC TestMain builds on the familiar <<mock-objects-servlet, "`mock`" implementations of
...
Jupiter-based example uses Spring MVC TestMain:
...
In Spring MVC TestMain, async requests can be tested by asserting the produced async value
...
There are no options built into Spring MVC TestMain for container-less testing of streaming
...
===== Spring MVC TestMain vs End-to-End Tests

Spring MVC TestMain is built on Servlet API mock implementations from the
...
There are pros and cons for each approach. The options provided in Spring MVC TestMain are
different stops on the scale from classic unit testing to full integration testing. To be
certain, none of the options in Spring MVC TestMain fall under the category of classic unit
...
Another important distinction when using Spring MVC TestMain is that, conceptually, such
...
sample tests] intended to show how to use Spring MVC TestMain. You can browse these examples
for further ideas. Also, the
https://github.com/spring-projects/spring-mvc-showcase[`spring-mvc-showcase`] project has
full test coverage based on Spring MVC TestMain.
...
* TestMain JavaScript within pages.
...
With Spring MVC TestMain, we can easily test if we are able to create a `Message`, as follows:
...
To use Selenium WebDriver with the Spring MVC TestMain framework, make sure that your project
...
----
	...
	// TestMain code that uses the above RestTemplate ...
...
Spring MVC TestMain's own tests include
...
* https://mockito.github.io[Mockito]: Java mock library based on the
  http://xunitpatterns.com/TestMain%20Spy.html[TestMain Spy] pattern.
