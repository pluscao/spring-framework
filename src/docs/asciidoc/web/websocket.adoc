...
the approach to testing controllers that handle HTTP requests by using the Spring MVC TestMain
framework -- that is, without running a Servlet container but relying on the Spring Framework
to invoke the annotated controllers. As with Spring MVC TestMain, you have two
